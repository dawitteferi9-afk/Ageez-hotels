 ...
 * Housekeeping and M5c with Maintenance. Plain server-rendered `<Link>`s
 ...
 *
 * Dashboard/Reservations/Rooms/Guests/Housekeeping/Maintenance are the
 * only implemented modules so far (per docs/SECURITY.md's RBAC matrix,
 * all five roles have at least "View" on each — Maintenance also gives
 * everyone "Report"). Services/Reports/Staff are approved scope but not
 * yet built — CLAUDE.md rule 8 / this phase's explicit scope boundary —
 * so they're listed but disabled rather than hidden, to be honest about
 * what the system will eventually contain without a dead link. No RBAC
 * check gates the nav items themselves: every implemented module grants
 * "View" (Maintenance also "Report") to every role, so nav visibility
