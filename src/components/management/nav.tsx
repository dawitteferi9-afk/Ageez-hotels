 ...
 * Housekeeping, M5c with Maintenance, and M4 Phase 5 with Services. Plain
 * server-rendered `<Link>`s (no active-route highlighting/client JS),
 * matching the guest site's `SiteHeader` philosophy of keeping navigation a
 * Server Component.
 *
 * Dashboard/Reservations/Rooms/Guests/Housekeeping/Maintenance/Services are
 * the only implemented modules so far (per docs/SECURITY.md's RBAC matrix,
 * all five roles have at least "View" on each — Maintenance also gives
 * everyone "Report"). Reports/Staff are approved scope but not yet built —
 * CLAUDE.md rule 8 / M4's explicit remaining scope gap (see
 * docs/V0.1_SCOPE.md) — so they're listed but disabled rather than hidden,
 * to be honest about what the system will eventually contain without a
 * dead link. No RBAC check gates the nav items themselves: every
...
const DISABLED_LINKS = ["Reports", "Staff"] as const;
